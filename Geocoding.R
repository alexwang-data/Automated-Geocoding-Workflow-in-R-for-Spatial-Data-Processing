...
df_clean <- df %>%
  mutate(
    ...
    Address = str_remove_all(                        # remove floor, suite, apartment, and unit
      Address,
      ",?\\s*(\\d+(st|nd|rd|th)\\s+Floor|Suite\\s+\\w+|Apt\\s+\\w+|Unit\\s+\\w+)"
    ),
...
# 10) write outputs
# delete_layer & delete_dsn = overwrite exisitng files
